...
class Cart extends Component {
    ...
    // removeAllCartItems() {
    //     CartItemsService.deleteAllCartItems().then(res => {
    //         alert("All Items from the cart are deleted successfully");
    //     });
    // }
    render() {
        ...
        return (
            <div>
                {allCartItems}
                <Link className="link" to='/'><button type="button" onClick={() => { }}>Place Order</button></Link>
            </div>
        )
    }
}
